...
//Initialize boolean values to adjust angle when making large turns.
bool _adjust = false;

//Initialize boolean values to get out of stuck positions.
bool _stuck = false;
...
// Hovercrafts's main entry point to begin sensing and moving.
void Move() {
...
// Verifies whether the hovercraft has not moved from one sensor reading to the next.
void CheckIfStuck() {
  if (_simTime > 5) {
...
// Sets the direction in which the hovercraft should move if it is stuck.
void MoveStuck() {
...
// Compares the five sensor distance readings and sets the hovercraft to react accordingly.
...
// Sets the servo position based on the sensor readings.
void SetServo() {
  if(_stuckGoLeft) {
...
// Converts the enum throttle values to floats that can be read in CoppeliaSim.
...
// Converts the enum servo motor position values to floats that can be read in CoppeliaSim.
...
// Helper function to CheckIfStuck() that stores the previous sensor values.
